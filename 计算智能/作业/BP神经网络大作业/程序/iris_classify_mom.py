...
def initialize_parameters(n_x, n_h, n_y):
    ...
    #初始化动量值
    v_dw1 = np.zeros(shape=(n_h,n_x))  #10X4
    v_db1 = np.zeros(shape=(n_h, 1))   #10X1
    v_dw2 = np.zeros(shape=(n_y, n_h)) #3X10
    v_db2 = np.zeros(shape=(n_y, 1))   #3X1
    # 通过字典存储参数
    parameters = {'w1': w1, 'b1': b1, 'w2': w2, 'b2': b2, 'v_dw1':v_dw1, 'v_db1':v_db1, 'v_dw2':v_dw2, 'v_db2':v_db2}
...
#gamma为衰减值，先取0.9看看效果
def update_parameters(parameters, grads, learning_rate=0.005, gamma = 0.9):
    w1 = parameters['w1']
    b1 = parameters['b1']
    w2 = parameters['w2']
    b2 = parameters['b2']
    
    v_dw1 = parameters['v_dw1']  #10X4
    v_db1 = parameters['v_db1']   #10X1
    v_dw2 = parameters['v_dw2'] #3X10
    v_db2 = parameters['v_db2']   #3X1
    ...
    # 更新参数
    v_dw1 = gamma * v_dw1 + dw1 * learning_rate  #10X4
    v_db1 = gamma * v_db1 + db1 * learning_rate #10X1
    v_dw2 = gamma * v_dw2 + dw2 * learning_rate #3X10
    v_db2 = gamma * v_db2 + db2 * learning_rate  #3X1
